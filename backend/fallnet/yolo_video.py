...
def create_dynamodb_record(cameraid, filename):
    fireid = random.randrange(1, 999)
    client = boto3.client(
        'dynamodb',
...
# frame dimensions
if not 'FILEURL' in os.environ:
    print("Error: Missing FILEURL ENV Variable")
    sys.exit()
fileurl = os.environ['FILEURL']
filename = os.environ['FILEKEY']

if not '_' in filename:
    print("Error: Invalid FILEKEY ENV Variable")
    sys.exit()
	...
	# boxes, idxs, idArray
		...
		# extract the bounding box coordinates
			...
			# draw a bounding box rectangle and label on the frame
				...
				if msg==2: 
                                    print("Person Fell.")
                                    create_dynamodb_record(camera_id, filename)
                                    sys.exit()
